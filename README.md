...
**Black Secret expansion (Phase 8 build):** Toggle from the New Game screen — the asymmetric expansion that lets one player take the role of Wu-Feng. At every Yin step 3, instead of the ghost auto-placing on its color board, Wu-Feng gets an intervention prompt: **place** the ghost normally, **summon** one of three demons (cost 2/3/4) into the catacombs (the ghost card pays the cost — its resistance must meet the demon's), or **throw a curse** matching the ghost's color (black is wild) at a chosen pyramid level. Adds the Calligrapher tile (replaces Night Watchman). Compatible with White Moon — run both for the full asymmetric+villager experience. See the rulebook's *Black Secret* topic for what's simplified (catacomb tokens + token effects, demon movement, per-curse effects, Bloody Mantra resolution, Blood Brothers, Shadow of Wu-Feng, skeleton placement, online seat assignment).
