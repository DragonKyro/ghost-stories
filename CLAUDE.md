...
- [x] **Phase 8** — Black Secret expansion (complete)
  ...
  - **Yin step 3** ([src/game/actions/yin.ts](src/game/actions/yin.ts)): when Black Secret is active and the drawn card is NOT an incarnation, pops the card off the deck, sets `phase: 'wuFengIntervention'`, stores card in `pendingArrivalCardId`, and returns. Incarnations fall through to the deterministic auto-place path.
  - **Wu-Feng handler** ([src/game/actions/blackSecret.ts](src/game/actions/blackSecret.ts)): `applyWuFengIntervene` dispatches on the choice:
    - **place** — validates color placement, places via base `placeGhost`.
    - **summon** — validates ghost-resistance-sum ≥ demon cost, moves demon to chosen entrance square.
    - **curse** — validates color match, validates pyramid, applies the *per-level effect* from `DEFAULT_CURSE_EFFECT_BY_LEVEL` (lvl 1 Qi loss, lvl 2 Tao discard, lvl 3 tile haunt, lvl 4 mass Qi).
    - **skeleton** — places a resistance-1 ghost (registered via `ensureSkeletonInRegistry`) on a chosen empty slot.
    - Demon Yin actions via `applyWuFengDemonActions` (move/search, dispatched before each player's Yin). Search reveals a token, then `applyCatacombToken` resolves Buddha (return demon), Blood of the Just (place Qi on Mantra), Cursed Tablet (highest-legal curse), Bones (+1 skeleton, capped at 3), Blood of Su-Ling (force-resolve a Mantra), Urn (urns++, on 3rd `spawnShadow` enters play).
    - **Bloody Mantras**: `placeQiOnMantra` is registered as a Qi-loss side-effect hook via `registerQiLossSideEffect`. Every Qi loss routes onto the lowest-level open Mantra. Filled Mantras `applyMantraResolution` (lvl 2 mass +1 Qi to all alive, lvl 3 `inactiveTaoMarker=false`, lvl 4 mass discard of strongest non-incarnation ghost on every board) then refresh as fresh cards of the same level.
    - **Blood Brothers** ([src/game/actions/yang.ts](src/game/actions/yang.ts)): `useTaoistPower` accepts the invocation when own board's power matches OR when actor is at 1 Qi AND opposite board's power (red↔green, blue↔yellow) matches AND opposite board is not possessed/blocked.
  - **Dispatcher** ([src/game/engine.ts](src/game/engine.ts)): `wuFengIntervene` case wired.
  - **UI**: NewGame Black-Secret toggle with Wu-Feng tag input ([src/ui/NewGame.tsx](src/ui/NewGame.tsx)); [src/ui/game/WuFengInterventionDialog.tsx](src/ui/game/WuFengInterventionDialog.tsx) renders when `phase === 'wuFengIntervention'` with three sections (Place / Summon / Curse) and live legality gating (summon costs ≤ resistance, curse colors filtered, max curse level from the pyramid); [src/ui/game/CatacombsPanel.tsx](src/ui/game/CatacombsPanel.tsx) sits in the side panel showing reserve demons, in-catacombs demons, curse pyramid by level, skeletons + mantras counts; Calligrapher tile glyph added.
...
- [x] **Phase 7** — White Moon expansion (complete)
  ...
  - **Family save rewards** ([src/game/actions/yang.ts](src/game/actions/yang.ts)): `saveVillager` calls `checkAndApplyFamilySaveReward` — when the family's saved count equals its size, the reward fires for the active player.
  - **Su-Ling cancellation** ([src/game/actions/yin.ts](src/game/actions/yin.ts)): `suLingCancelsGhost(state, board, space)` returns true if Su-Ling sits on that slot; Yin-phase center abilities (Haunter, Tormentor, Devourer) all skip for that ghost while she's there. Curse-die rolls are still CONSUMED from the payload for index alignment but their effect is discarded.
  - **Villager fleeing** ([src/game/actions/yin.ts](src/game/actions/yin.ts)): `applyHaunterFlee` runs on every card→stone1 advance. Top villager on the first line tile moves to the next tile in line. If destination is off-board, haunted, or full (3), the villager dies.
  - **Villager move-with-Taoist** ([src/game/actions/yang.ts](src/game/actions/yang.ts)): `moveTaoist` accepts a `carryVillager: boolean` flag; pops the top villager from the actor's prior tile and pushes onto the destination's stack. Validates ≤ 2 prior villagers + non-haunted destination.
  - **Receptacles + Mystic Barrier** ([src/game/actions/yang.ts](src/game/actions/yang.ts)): `placeMoonCrystal` requires the actor to stand on the corner village tile that matches the receptacle. Setting the 4th receptacle flips `mysticBarrierPending: true`. `endYangPhase` checks the flag and runs `applyMysticBarrier` — discards the highest-resistance non-incarnation ghost on every non-neutral board (no rewards/curses), returns the 4 crystals to reserve, clears the trigger.
  - **Move Su-Ling** action ([src/game/actions/yang.ts](src/game/actions/yang.ts)): `moveSuLing` places her on any empty haunting icon. Required to be the active player.
